...
namespace Gmich.Cedrus.Rendering
{
    public sealed class RenderedComponent
    {
        ...
        public RenderedComponent(Func<Texture2D> texture, Func<Rectangle> textureRectangle, Func<Vector2> position, Func<Color> color, Func<float> rotation, Func<Vector2> scale, Func<Vector2> origin, Func<SpriteEffects> effects, Func<float> layer)
        {
